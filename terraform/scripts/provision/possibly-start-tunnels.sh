...
start_tunnel() {
    ...
    nohup bash -c "until (ssh -M -T -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i .ssh/tunnel -N -L 0.0.0.0:${port}:localhost:${port} ubuntu@${eip}); do echo 're-establishing tunnel to geth ${gid}:${port}'; done" &
}
